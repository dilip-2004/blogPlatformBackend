...
async def create_blog(blog: BlogCreate, current_user: UserInDB = Depends(get_current_user)):
    ...
    await socket_manager.emit_blog_update("created", {
...
async def get_blogs(
    ...
    # Fetch username for each blog using user_id
    for blog in blogs:
...
async def update_blog(
    ...
    # Fetch username using user_id foreign key
    author = await db.users.find_one({"_id": ObjectId(updated_blog["user_id"])}, {"username": 1})
    updated_blog["username"] = author["username"] if author else "Unknown"

    # Emit socket event for real-time dashboard updates
    await socket_manager.emit_blog_update("updated", {
        "id": blog_id,
        "title": updated_blog["title"],
        "tags": updated_blog["tags"],
        "published": updated_blog["published"],
        "user_id": current_user.id
    })
...
async def delete_blog(blog_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    # Emit socket event for real-time dashboard updates
    await socket_manager.emit_blog_update("deleted", {
        "id": blog_id,
        "title": blog["title"],
        "tags": blog["tags"],
        "published": blog["published"],
...
async def search_blogs(
    ...
    recommendations = []
    for blog in blogs:
        author = await db.users.find_one({"_id": blog["user_id"]}, {"username": 1})
